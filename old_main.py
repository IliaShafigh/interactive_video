...
class Game:
    def __init__(self):
        ...
        # Define Nodes
        self.intro_node = Node(
            "assets/example/intro.mp4",
            {"Home": "home_node"}
        )
        self.home_node = Node(
            "assets/example/home.jpg",
            {'Video': 'video_node', 'Picture': 'picture_node', 'Example': 'example_node', 'Quit': 'quit'},
            state_actions={'set': {'energy': 3, 'mood': 3, 'alcohol': 3}},# Beispiel: setzen der Energie zu bginn des spiels
            is_video=False
            )
        self.video_node = Node("assets/example/1080p60_5sec.mp4",
                                   {"Home": 'home_node'})
        self.picture_node = Node(
            "assets/example/icon.jpg",
            {'Example': 'example_node', 'Picture': 'picture_node', 'Home': 'home_node'},
            state_actions={'increase': {'mood': 1}},
            is_video=False
        )
        self.example_node = Node(
            "assets/example/untitled.jpeg",
            {'Energy == 5': 'test1_node', 'Energy > 5': 'test2_node', 'Mood > 5': 'test3_node', 'Mood == 5': 'test4_node', 'Example': 'example_node', 'Picture': 'picture_node', 'Home': 'home_node'},
            conditions={
                'Energy == 5': ('energy', lambda x: x == 5),
                'Energy > 5': ('energy', lambda y: y > 5),
                'Mood > 5': ('mood', lambda y: y > 5),
                'Mood == 5': ('mood', lambda y: y == 5)
                },
            state_actions={'increase': {'energy': 1}}  # Beispiel: Erhöhen der Energie um 1 beim Übergang zu dieser Node
        )
        self.test1_node = Node(
            "assets/example/slip_die.mp4",
            {'Home': 'home_node', 'Example': 'example_node'},
        )
        self.test2_node = Node(
            "assets/example/voiceover.mp4",
            {'Home': 'home_node'},
        )
        self.test3_node = Node(
            "assets/example/voiceover.mp4",
            {'Home': 'home_node'},
        )
        ...
        self.nodes = {
            ...
            'video_node': self.video_node,
            'picture_node': self.picture_node,
            'example_node': self.example_node,
            'test1_node': self.test1_node,
            'test2_node': self.test2_node,
            'test3_node': self.test3_node,
            'quit': None
    ...
    def show_choices(self, choices):
        ...
        for choice_text in choices:
            ...
            if choice_text in self.current_node.conditions:
                condition_key, condition_func = self.current_node.conditions[choice_text]
                show_button = condition_func(self.game_state.get_state(condition_key))
